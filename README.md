...
| git add _filename_ | 添加到暫存區 (untracked -> tracked) |
| git commit | 提交到本地端倉庫 |
| git log <br> git log --oneline | 查看本地端倉庫的提交歷史紀錄|
| git rm --cache _filename_ <br> git restore --staged _filename_ | 取消添加到暫存區 (tracked -> untracked) |
...
| git rm _filename_ | 刪除文件, 資料狀態: 工作區(✕) 暫存區(✕) |
| git rm --cached _filename_ | 刪除文件, 資料狀態: 工作區(✓) 暫存區(✕) |
